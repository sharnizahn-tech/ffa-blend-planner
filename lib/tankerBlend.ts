...
// together with clean stock from other tanks, so the COMBINED load's FFA
// averages back under the limit. The engineer picks one of three ranked
// options — how much of the problem tank each clears trades off against
// how much safety margin is left under the limit, since this blend can't
// be lab-checked until it's already loaded, so it's less precise than
// blending in a tank first.
 ...
 *  spare clean stock elsewhere, works out up to three ranked ways to blend
 ...
 *  well clear of the limit). Returns null when there's no problem tank;
 *  returns an empty options list when there's no clean stock anywhere to
 *  blend it with. */
