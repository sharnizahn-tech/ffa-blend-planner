...
export const adviseRequestSchema = z.object({
  ...
  tankerBlend: z
    .object({
      ...
      // Which option the engineer has actually ticked in the UI right now
      // — null only if the suggestion just appeared and nothing's ticked
      // yet, which shouldn't normally happen since the UI defaults to one.
      selectedRisk: z.enum(["max", "balanced", "safest"]).nullable(),
      options: z.array(
        z.object({
...
- What the data fields mean, and what to call them in your response:
  ...
  - "batchBlend" is a day-by-day tank-to-tank transfer plan to bring existing stock to good FFA with no new incoming CPO — call it "the blend-down plan". State whether it's feasible, over how many days, exactly as given. This is only ONE of two ways the engineer can actually blend: they also have a manual one-off transfer tool (pick any source tank, any destination tank, any amount, right now) that isn't itself a computed "plan" — when asked how much to blend today, consider both: recommend a specific source tank → destination tank → amount using the same lowest-FFA-source / highest-FFA-destination logic "batchBlend" itself uses, even if no formal multi-day plan is shown. If NEITHER a tank-to-tank transfer NOR the blend-down plan gets the load compliant (no clean stock left, or not enough transfer capacity/time), only then bring up blending directly at the tanker — loading from multiple source tanks into the same tanker so the combined load's FFA averages out; use the exact numbers from "tankerBlend" below when it's present, never your own split. State the ticked option ("selectedRisk") as a direct, decisive instruction ("load X MT from A with Y MT from B") — the engineer already chose it in the app, so don't re-litigate the choice unless one of the other two listed options is genuinely a better trade-off worth flagging. Still name the real reason it's used only here and not routinely: there's no lab check on the blend until it's already loaded, so it's less precise than blending in a tank first. Never present it as just another equal option next to tank-to-tank blending or holding, and never suggest it while a tank-to-tank option is still available.
  ...
  - "tankerBlend" is the ONLY source of real numbers for blending at the tanker — never invent your own tanker-blend split. When it's null, don't bring up tanker-blending at all (a tank-to-tank blend is still available, or there's no problem tank right now). When it's present, "problemTank" is the tank that's over the limit and can't be cured in a tank first, and "options" holds up to three real, already-computed ways to blend it at the tanker ("max" clears the most of it but lands right at the limit with zero margin; "balanced" and "safest" clear less but leave real headroom, since this blend can't be lab-checked before the tanker leaves) — each with exactly how many MT from the problem tank plus how many MT from which clean tank(s), the resulting FFA, and (when a buyer profile is set up) the RM penalty with vs without it. "selectedRisk" is the ONE the engineer has actually ticked in the app right now — lead with that one and quote its numbers as the current plan; only mention another option from the list if it's genuinely worth flagging as a better trade-off. Quote every number exactly, and always repeat the risk framing from the rule above (no lab check until it's loaded).
